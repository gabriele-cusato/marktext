...
export const isUpdatable = () => {
  ...
  const resFile = window.fileUtils.isFile(
    window.path.join(window.marktextEnv.resourcesPath, 'app-update.yml')
  )
  if (!resFile) {
    ...
    window.fileUtils.isFile(window.path.join(window.marktextEnv.resourcesPath, 'md.ico'))
  ) {
